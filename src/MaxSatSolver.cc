...
Glucose::EnumOption option_maxsat_strat("MAXSAT", "maxsat-strat", "Set optimization strategy.",
                                        "one|one-2|one-neg|one-wc|one-neg-wc|one-pmres|one-pmres-2|pmres|pmres-reverse|pmres-log|pmres-split-conj|kdyn");
Glucose::EnumOption option_maxsat_disjcores("MAXSAT", "maxsat-disjcores", "Set disjunct unsatisfiable cores policy.",
                                            "no|pre|all", 1);
...
Glucose::EnumOption option_core_reduction("MAIN", "core-reduction", "Core reduction algorithm to be applied\n", "progression|mxp|smxp");
...
//}
    ...
    MaxSatSolver::MaxSatSolver() : lowerbound(0), lastConflict(0), lastPropagation(0), lastCallCpuTime(0.0) /*, timeBudget(0.0)*/ {
        ...
        if(strcmp(option_core_reduction, "progression") == 0)
            minStrat = &MaxSatSolver::progressionMinimize;
        else if(strcmp(option_core_reduction, "mxp") == 0)
            minStrat = &MaxSatSolver::mergexplainMinimize;
        else if(strcmp(option_core_reduction, "smxp") == 0)
        ...
        for (int i = 0; i < sizes.size(); i++) {
            ...
            for (int j = 0; j < clauses.size(); j++) {
                ...
                for (int k = 0; k < clause.size(); k++) {
                    ...
                    if (weights[var(clause[k])] == 0 || signs[var(clause[k])] == sign(clause[k])) {
                        min = LONG_MAX;
                        break;
                    }
        ...
        for (int i = 0; i < softLiterals.size(); i++) {
            if (value(softLiterals[i]) != l_Undef) {
                if (value(softLiterals[i]) == l_False) {
                    trace(maxsat, 20, "False soft literal at level 0: " << softLiterals[i] << " of weight " <<
                                      weights[var(softLiterals[i])]);
...
//        this->dump();
    ...
    void MaxSatSolver::solve_() {
        ...
        for (; ;) {
            ...
            trace(maxsat, 2,
                  "Solve with " << assumptions.size() << " assumptions. Current bounds: [" << lowerbound << ":" <<
                  upperbound << "]");
            trace(maxsat, 100, "Assumptions: " << assumptions);

            if (assumptions.size() == 0 && upperbound != LONG_MAX)
            {
                dbg("solve_ assumptions empty, upper bound not reached")
                status = l_Undef;
            }
            else //progressionBinaryFind(limit);
            {
                dbg("solve_ else pbs::solve");
                PseudoBooleanSolver::solve(); //
            }
...
//*
        while (lowerbound + limit < upperbound) {
            ...
            trace(maxsat, 15, "Find: progress to " << progression << "; fixed = " << fixed << "; LBD ratio: " <<
...
//*
        ...
        while (lowerbound + limit < upperbound) {
            ...
            trace(maxsat, 15, "Find: try with " << fixed << "+" << toAdd << " literals; LBD ratio: " <<
                              (sumLBD / conflictsRestarts));
...
//            progression = progressionFrom;
    ...
    void MaxSatSolver::progressionBinaryMinimize(int64_t limit) {
        ...
        while (lowerbound + limit < upperbound) {
            ...
            trace(maxsat, 15,
                  "Minimize: progress to " << progression << "; LBD ratio: " << (sumLBD / conflictsRestarts));
        ...
        while (lowerbound + limit < upperbound) {
            ...
            trace(maxsat, 15, "Minimize: try with " << fixed << "+" << toAdd << " literals; LBD ratio: " <<
                              (sumLBD / conflictsRestarts));
...
//}
    ...
    void MaxSatSolver::binaryMinimize(int64_t limit) {
        ...
        trace(maxsat, 10,
              "Minimize core of size " << conflict.size() << " (" << (conflicts - lastConflict) << " conflicts; " <<
              static_cast<int>(cpuTime) << " seconds; each check with budget " << budget << ")");
        trim();
        if (budget == 0) return;
    ...
    void MaxSatSolver::minimize() {
        ...
        for (; ;) {
            ...
            trace(maxsat, 10,
                  "Try to minimize with " << assumptions.size() << " assumptions (" << requiredAssumptions <<
                  " required, ie. " << 100 * requiredAssumptions / (assumptions.size() + 1) << "% of the core; " <<
                  nUnknown << " unknowns, ie. " << 100 * nUnknown / (assumptions.size() + 1) << "% of the core)");
            setConfBudget(budget);
            PseudoBooleanSolver::solve();
...
//                addClause(lits);
    ...
    void MaxSatSolver::dump(std::string msg) {
        cout << set_color(CYAN) << "[ D U M P (Solver) " << msg << " ]" << endl;
        ...
        cout << "Conflicts: " << this->conflict << endl;
